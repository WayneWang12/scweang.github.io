...
其实，按道理讲，如果面向对象语言和函数式语言水火不相容的话，那么任何纯粹的函数式语言程序员都不会屑于使用面向对象语言，任何面向对象语言的支持者也会坚决抵制函数式编程。但是其实面向对象和函数式编程并不是水火不相容的，因此才会有那么多的多范式语言力图结合好面向对象和函数式。当然，这两种范式毕竟是有差别的，只是这种差别并没有像前面说的那样那么夸张，夸张到出现一门结合两者的语言就会有人过来烧死异教徒。

对于Scala来讲，它跟之前对函数式编程有部分支持的面向对象语言的差别在哪里呢？那就是你可以使用Scala来实现“纯函数式编程”。

什么是纯函数式编程？一个程序是纯函数式的，如果你只用纯函数构建这个程序。

那什么是纯函数呢？一个函数是纯的，如果这个函数没有副作用。

那什么是副作用呢？如果一个函数在执行的过程中产生了除了根据给定输入计算结果并返回以外的可以被观测到的影响，这种影响就叫做副作用。

那么，如果一个函数不是纯的，我们可能会观测到什么样的影响呢？如下：
...
说到这里，很多没有接触过函数式编程的朋友可能就炸毛了。不能修改变量、不能修改对象、不能抛出异常、不能输入输入、不能读写文件、不能绘制图形，那写什么程序啊！这样搞的话，函数式编程有个毛用呢！

嘿嘿，先别着急，函数式编程的用途可大着呢。

在这里，我得先纠正大家在学习函数式编程找资料的时候经常会被误导的两个概念：

###### 1. 函数式编程不能修改变量，以至于只能使用递归，不能使用迭代。
**函数式编程不在乎是否修改变量，只在乎这个变量的修改是否会在函数外被观测到。**

###### 2. 函数式编程没有副作用。
**函数式编程是这样一种编程方法，它教会你如何尽量消除或推迟副作用，最终得到一个纯函数的程序，或者一个包含着纯函数的核心，只在最外面的很薄的一层处理副作用的程序。副作用需要靠程序员遵循一定的原则来消除，而不是语言帮你做这件事情(当然Haskell等纯函数式编程语言除外)。**

在Scala这一块，就有一些程序员利用Scala来实现了一个纯函数的类库[scalaz](https://github.com/scalaz/scalaz)，以作为Scala标准库的补充。

不过，我必须特别地指出，在学习Scala的过程中，我并没有看到Martin Odersky对函数式编程有多执着。Coursera的公开课上讲Scala的时候，他写的程序引进了一个有副作用的操作之后，他只是淡定地讲这样做会使得程序失去引用透明性和替代模型（这两个概念我后来写博文讲），而并没有执着于将纯的特性持续下去；而如果你点进Scala的标准库里面看一下部分实现、尤其是集合类库的那部分，Scala很多时候是用的可变的局部变量来实现一些例如map、fold、reduce这样的函数式的操作，他也为了效率而没有追求纯的函数式方法（没有执着于递归）。因为Scala毕竟是一门多范式的语言，你可以利用它的各种特性来增加开发效率和程序执行效率，而并不需要执着于必须面向对象或者必须函数式。

但是，学会函数式编程对我们来讲，会是大有裨益的。如果努力去追求函数式编程，会大大增加我们程序的并发性、可扩展性、可测性与稳定性。而这些促进程序拥有这些良好特性的方法是我们平时很少接触、或者很少去实践的。所以本博客主要是普及使用Scala进行函数式编程的一些方法。并且穿插地会讲Scala相关技术栈的介绍与应用。当然，响应式也是必不可少的。只是精力有限，一周或者两周才能写好一篇博客。只能希望我可以坚持更新本博客或者公众号两到三年的时间，这样才能比较好地介绍清楚我所了解的方方面面，也能帮我对自己所了解的知识有一个更加深入的理解。不过毕竟也是边学边讲，所以如果看到文章里面有什么错误，还是希望能够通过我的公众号回复给我。预先感谢大家的指正。

### 总结

Scala是一门神奇的语言，也是一门现象级的语言。它依托于JVM，首先就有了近乎海量的Java类库的支持；然后实践了一个更加纯粹、更加简洁、更少样板代码的面向对象语言，帮助Scala程序员大大提高了开发效率；再之，它利用了函数对象以及贴心的语法糖，使得语言可以完全实践函数式的编程方法，因而给予程序员足够的空间写出具有更多良好特性的程序。然后强静态类型带来的编译期检查、跟Java几乎相等的执行效率，以及Akka、Kafka、Play、Spray、Slick、Spark等基于Scala构建的优秀的开源框架，都使得Scala在未来的IT世界会有更多更广泛的应用。

所以，如果你想学一门编程语言，你想依靠语言来学习更多的编程思想和计算机科学里面的概念，或者你想玩一下大数据，用Spark来做一下数据分析什么的，来学Scala吧，准没错！

这是一门神奇的语言，在学习和使用的过程中，充满着乐趣。
